...
public class ParseTree {
	private Pair root;
	private Pair end;
	private ArrayList<Pair> stack;// stack holds all the left pairs after
									// parenthesis
	...
	public Pair add(Token lst) {
		...
		if (lst.getValue().compareTo("\\)") == 0) {
			end = stack.remove(stack.size() - 1); // base case closing back up
													// and resets end to last
													// part of stack
		}
		if (lst.getValue().compareTo("\\(") == 0) {
			Pair left = new Pair();// new list in left pair
			stack.add(end.add(left));// adds left pair to end, and adds right
										// pair to stack
		} else {
