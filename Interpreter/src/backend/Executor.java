...
public class Executor // The Class That Runs Scheme Code
{
	...
	public Pair cons(Pair p) {
		...
		{
			if (((Token) p.getCar()).getValue().charAt(0) == '\'') // Check the
																	// definition
																	// in the
																	// symbol
																	// table
			...
			// if that token is naked then check its definition in the symbol
			// table and return that definition
			{
	...
	public boolean testSymbol(Pair p) {
		...
		for (int i = 0; i < specialchars.length; i++) {
			if (p.getCdr().isAtom()
					&& ((Token) ((Atom) p.getCdr())).getValue().equals(
							specialchars[i])) {
				return true;
			}
		}
	...
	public boolean testDouble(Pair p) {
		...
		if (Double.parseDouble(((Token) ((Atom) p.getCdr())).getValue())
	...
	public boolean testChar(Pair p) {
		if (p == null) {
			return false;
				...
				.isLetter(((Token) p.getCar()).getValue().charAt(0))) {
			return true;
		}
		return false;
	}
	...
	public boolean testEqualSymbol(Pair p) {
		...
		if (p.getCdr() == null) {
			...
			return true;
		} else if (((Token) p.getCar()).getValue().equals(
				((Token) p.getCdr().getCar()).getValue())) {
	...
	public int powIntegerMethod(Pair p) {
		if (p == null) {
			...
			return (int) Math.pow(
					Double.parseDouble(run((Pair) p.getCar()).toString()),
					(double) multiplyIntegerMethod(p.getCdr()));
		} else {
			return (int) Math.pow(
					Double.parseDouble(((Token) p.getCar()).getValue()),
					(double) multiplyIntegerMethod(p.getCdr()));
		}
	}

	public double addNumberMethod(Pair p) {// use root can add as many as we
											// need to -recursive...base case!!!
											// cdr empty add 0

		if (p == null) {
			return 0.0;
		} else if (!p.getCar().isAtom()) {
			return Double.parseDouble(run((Pair) p.getCar()).toString())
					+ addNumberMethod(p.getCdr());// will run() return int for
													// this?
		} else {
			return Double.parseDouble(((Token) p.getCar()).getValue())
